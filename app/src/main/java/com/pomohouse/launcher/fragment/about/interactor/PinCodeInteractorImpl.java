...
public class PinCodeInteractorImpl extends BaseInteractor implements IPinCodeInteractor {
    ...
    public void callGetCode(ImeiRequest imeiRequest, OnPinCodeListener listener) {
        ...
        Observable<ResultGenerator<PinCodeModel>> service = pairService.callPinCode(imeiRequest).subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread());
        ...
        listener.subscriptionRetrofit(service.subscribe(new Observer<ResultGenerator<PinCodeModel>>() {
            ...
            public void onNext(ResultGenerator<PinCodeModel> readyModel) {
                ...
                else listener.onPinCodeFailure(metaData);
            }
        }));
    }

    @Override
    public void callQRCode(ImeiRequest imeiRequest ,OnQRCodeListener listener) {
        if (listener == null) return;
        Observable<ResultGenerator<QRCodeModel>> service = pairService.callRequestQRCode(imeiRequest).subscribeOn(Schedulers.newThread()).observeOn(AndroidSchedulers.mainThread());

        listener.subscriptionRetrofit(service.subscribe(new Observer<ResultGenerator<QRCodeModel>>() {
            @Override
            public void onCompleted() {
                listener.unSubscriptionRetrofit();
            }
            ...
            @Override
            public void onError(Throwable e) {
                listener.onQRCodeFailure(MetaDataNetwork.getError(e));
            }

            @Override
            public void onNext(ResultGenerator<QRCodeModel> readyModel) {
                metaData = buildMetadata(readyModel.getResCode(), readyModel.getResDesc());
                if (metaData.getResCode() == 0)
                    listener.onQRCodeSuccess(metaData, readyModel.getData());
                else listener.onQRCodeFailure(metaData);
            }
        }));
    }
}
